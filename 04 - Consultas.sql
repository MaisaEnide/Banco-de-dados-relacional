...
2) Obtenha a maior idade, a menor idade e a idade média dos alunos do curso de 'Ciencia da Computacao'
    ...
    SELECT MAX(A.Idade) AS MaxIdade, MIN(A.Idade) AS MinIdade, AVG(A.Idade) AS MediaIdade
    FROM Curso C INNER JOIN Aluno A
    ON (C.Cod_Curso = A.Cod_Curso)
    WHERE (C.Nome_Curso = 'Ciencia da Computacao');
--------------------------------------------

3) Obtenha o nome de cada curso (que tenha aluno) e, para cada curso, a média de idade e a quantidade
de alunos
...
4) Obtenha o nome de cada aluno da universidade (mesmo aqueles que não cursaram disciplinas ainda) e
a quantidade de disciplinas que cada um já cursou. Retorne em ordem crescente da quantidade de disciplinas e, se houver empate na quantidade, em ordem crescente de nome do aluno
...
5) Obtenha o nome das disciplinas que a aluna 'Claudia' cursou e a nota que ela tirou em cada uma das
disciplinas
...
8) Obtenha o nome dos alunos (de qualquer curso) que tem idade superior à média de idade dos alunos do
